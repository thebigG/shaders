...
void main()
{
    ...
    vec3 rainbow = mix(red, orange, step(0.14, st.x));

    rainbow = mix(rainbow, yellow, step(0.28, st.x));
    rainbow = mix(rainbow, green, step(0.42, st.x));
    rainbow = mix(rainbow, blue, step(0.56, st.x));
    rainbow = mix(rainbow, indigo, step(0.70, st.x));
    rainbow = mix(rainbow, violet, step(0.84, st.x));
