...
{
    return smoothstep(point, point + 0.02, input_edge) -
           smoothstep((point + range), (point + range) + 0.02, input_edge);
}
...
float degrees_to_scalar(in float angle) { return ONE_DEGREE * angle; }
...
void main()
{
    ...
    // radius_multiplier = select_from_range_smooth(degrees_to_scalar(begin_point_degrees),
    // degrees_to_scalar(range_degrees), angle_x);

    radius_multiplier = select_from_range(degrees_to_scalar(begin_point_degrees),
                                          degrees_to_scalar(range_degrees), angle_x);

    //  radius_multiplier = select_from_range(selected_color,selected_color  +
    //  degrees_to_scalar(15.00), angle_x); radius_multiplier =
    //  select_from_range(selected_color,degrees_to_scalar(range_degrees) , angle_x);
