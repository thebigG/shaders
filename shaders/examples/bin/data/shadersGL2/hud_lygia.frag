...
// click uniform samplerXX iChannel0..3;          // input channel. XX = 2D/Cube uniform vec4 iDate;
...
// Same as movingLine_lygia but it uses gl_FragCoord.xy so it is NOT relative (it is using absolute
// coords)
...
float line_absolute_lygia(vec2 uv, vec2 center, float radius)
{
    ...
    return rotated_line_right_bottom + rotated_line_left_top + rotated_line_left_bottom +
           rotated_line_right_top;
}
...
// uv: current coord
// center: center of frame/rect(e.g. u_resolution.xy/2.0). The circle will be drawn relative to this
// center point radius: The radius of the circle relative to center
...
float circle2_lygia(vec2 uv, vec2 center, float radius, float stroke_width, float opening)
{
    ...
    if ((uv.y) < (center.y + ((radius * 0.05) * opening)) &&
        ((uv.y) > (center.y - ((radius * 0.05) * opening))))
    {
        full_circle = 0.0;
    }
...
float circle3_l_and_r_arcs_lygia(vec2 uv, vec2 center, float radius, float width)
{
    ...
    // Remove top arc
    if (is_uv_in_gap2(uv, (center + radius * 0.07), (radius * 0.069 * opening)))
    {
        full_circle = 0.00;
    }

    // // Remove bottom arc
    if (is_uv_in_gap3(uv, (center - radius * 0.07), (radius * 0.069 * opening)))
    {
        full_circle = 0.00;
    }
...
float circle3_t_and_b_arcs_lygia(vec2 uv, vec2 center, float radius, float width)
{
    ...
    // Remove unwanted circle areas
    // Remove right arc
    if (is_uv_in_gap2_x(uv, (center + radius * 0.14), (radius * 1.7 * opening)))
    {
        full_circle = 0.00;
    }
    ...
    // // Remove unwanted circle areas
    // // Remove left arc
    if (is_uv_in_gap3_x(uv, (center - radius * 0.14), (radius * 1.7 * opening)))
    {
        full_circle = 0.00;
    }
...
{
    return left_triangle(xy, center) + right_triangle(xy, center) + top_triangle(xy, center) +
...
float bip2(vec2 uv, vec2 center)
{
    ...
    return (0.5 - 0.5 * cos(30.0 * u_time)) * SMOOTH(r, 5.0) + SMOOTH(6.0, r) - SMOOTH(8.0, r) +
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    finalColor +=
        (circle(uv, c, 150.0 * (20.0 / 3.0), 0.001) + circle(uv, c, 248.0 * (20.0 / 3.0), 0.001)) *
        blue1;
    ...
    finalColor +=
        circle3_t_and_b_arcs_lygia(uv, c, 3000.0, 0.0020) * blue1 * 0.5; // top and bottom arcs
    ...
    finalColor +=
        0.7 *
        circle2_lygia(uv, c, 870.000 * 3.00, 0.000725, smoothstep(-1.0, 1.0, cos(u_time)) + 0.6) *
        blue3;
